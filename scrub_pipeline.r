...
# Examine the first few lines to quickly see what data types are contained
print(readLines(path_and_file_name, n=5))

# Use readr for fast reading and inferring data types
require(readr)
data_set <- read_csv()
str(data_set)
...
Binarize_Features <- function(data_set, features_to_ignore=c(), leave_out_one_level=FALSE, max_level_count=20) 
{
  ...
  for (feature_name in setdiff(text_features, features_to_ignore)) 
  {
    ...
    if (length(unique(feature_vector)) == 1) next
...
Feature_Engineer_Dates <- function(data_set, remove_original_date=TRUE) 
{
  ...
  for (feature_name in date_features) 
  {
    ...
    if (remove_original_date) data_set[, feature_name] <- NULL
  }
  return(data_set)
}

# Feature Engineer Integers
...
# Identify Outliers
Identify_Outliers <- function(data_set, features_to_ignore=c(), outlier_sd_threshold = 2, remove_outlying_features = FALSE)
{
  ...
  for (feature_name in setdiff(names(data_set),features_to_ignore)) 
  {
    ...
    if (outlier_count > 0) 
    {
      ...
      if (remove_outlying_features) data_set[, feature_name] <- NULL
    }
  }
  outliers <- data.frame(outliers) %>% rename(feature_name=X1, outlier_count=X2) %>% mutate(outlier_count=as.numeric(as.character(outlier_count))) %>% arrange(desc(outlier_count))
